...
def get_connections(g, skeleton, eroded, img):
    hier, contours, hierarchy = cv2.findContours(skeleton.copy(),
                                                 cv2.RETR_CCOMP,
    ...
    for i in areas:
        ...
        blank_image = cv2.drawContours(blank_image, [contour], 0,
                                       (255, 255, 255), 1)
        ...
        arc_length = cv2.arcLength(contour, True) / 2
        x0, y0 = float(x + arc_length * vx * .5), \
            float(y + arc_length * vy * .5)
        x1, y1 = float(x - arc_length * vx * .5), \
            float(y - arc_length * vy * .5)
        ...
        if check_los(eroded, g.nodes[node_a].pos, g.nodes[node_b].pos):
            ...
            cv2.line(img, g.nodes[node_a].pos, g.nodes[node_b].pos,
